...
int board_late_init(void)
{
	...
	sprintf(&beaglev_mac_addr[1], "%pM", mac_addr);

	beaglev_mac_addr[18] = ']';
	beaglev_mac_addr[19] = '\0';

	for (idx = 0; idx < 20; idx++) {
		if (beaglev_mac_addr[idx] == ':')
			beaglev_mac_addr[idx] = ' ';
	}
	env_set("beaglev_mac_addr0", beaglev_mac_addr);
	...
	env_set("beaglev_mac_addr1", beaglev_mac_addr);
